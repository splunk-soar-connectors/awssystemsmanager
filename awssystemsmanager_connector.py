...
class AwsSystemsManagerConnector(BaseConnector):
    ...
    def _create_client(self, action_result, param=None):
        ...
        try:
            if self._access_key and self._secret_key:
                ...
                self._client = client("ssm", region_name=self._region, config=boto_config)
        except Exception as e:
    ...
    def _create_s3_client(self, action_result, param=None):
        ...
        try:
            ...
            return self.set_status(phantom.APP_ERROR, f"Could not create boto3 client: {e}")
            ...
            try:
                ...
                if file_name:
                    success, message, vault_id = ph_rules.vault_add(
                        file_location=file_path, container=self.get_container_id(), file_name=file_name
                    )
                ...
                # as either 'stdout' or 'stderr' into the vault
                    ...
                    success, message, vault_id = ph_rules.vault_add(
                        file_location=file_path, container=self.get_container_id(), file_name=os.path.basename(output_s3_object_key)
                    )
                    result_json["filename"] = os.path.basename(output_s3_object_key)
            except Exception as e:
                return action_result.set_status(phantom.APP_ERROR, f"Could not file to vault: {e}")

            if not success:
                return action_result.set_status(phantom.APP_ERROR, f"Could not save file to vault: {message}")
    ...
    def _handle_send_command(self, param):
        ...
        try:
            ret_val, resp_json = self._get_s3_object(
                action_result, output_s3_bucket_name, output_s3_object_key, save_output_to_vault, file_name, param
            )
        except Exception:
    ...
    def _handle_run_document(self, param):
        ...
        try:
            ...
            return RetVal(action_result.set_status(phantom.APP_ERROR, f"Invalid JSON for Parameters. Error: {e!s}"), None)
        working_directory = param.get("working_directory")
        timeout_seconds = param.get("timeout_seconds")
        comment = param.get("comment")
    ...
    def _handle_get_parameter(self, param):
        ...
        self.debug_print("Making get_parameter call {} decryption".format("with" if with_decryption else "without"))

        args = {"Name": name, "WithDecryption": with_decryption}
    ...
    def _handle_add_parameter(self, param):
        ...
        args = {"Name": name, "Value": value, "Type": type, "Overwrite": overwrite}
    ...
    def _handle_describe_instance(self, param):
        ...
        args = {"InstanceInformationFilterList": instance_information_filter_list}
        ...
        self.debug_print("Found {} instances in describe_instance response".format(len(response["InstanceInformationList"])))

        if len(response["InstanceInformationList"]) == 0:
            return action_result.set_status(
                phantom.APP_ERROR, "No SSM instance found. Please check if instance is assigned to a System Manager IAM role."
            )
